...
in such a way,
1. to close the loop, set the last ```ring``` of ```Ringtrain``` as identical to the first.# einzellmill
## numerical simulation for moving einzell lens field
The simulation set up is a series of parallel conducting rings.
Each carrying a 10MHz, 30kV, 3-phase potential, so as to create a moving electric field.
...
##### 1. Establish all mutual capacities with ```setMutualCapacities()```. Invoking ```capacityOfRings()``` for all pairs of rings.
